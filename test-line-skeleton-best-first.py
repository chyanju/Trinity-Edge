...
if __name__ == "__main__":
    ...
    # costs for each skeleton
    scosts = [
        1
    ]
    # costs for each terminal rules
    # should correspond with priority_pools
    pcosts = [
        [
            # hole0
            [2, 5], # cost for [-99,-2] is 2, and cost for [-99,-1] is 5
            # hole1
            [1, 3, 10, 5],
            # hole2
            [2]
    ...
    # you can specify the search order here
    # each item is (skeleton id, combination)
    # a combinnation is a tuple of indices pointing to priority_pool
    # see example below
    iccs = [
        (0, (0,1,0)), # this is: select(separate(gather(@param0, ['-99', '-2']), 3), ['-3'])
        (0, (0,3,0)), # this is: select(separate(gather(@param0, ['-99', '-2']), 2), ['-3'])
        (0, (0,2,0)),
        (0, (0,0,0)),
        (0, (1,1,0)), # this is: select(separate(gather(@param0, ['-99', '-1']), 3), ['-3'])
        (0, (1,3,0)),
        (0, (1,2,0)),
        (0, (1,0,0)),
    ]
    # or you can automatically generate a best-first order by sorting out the (e.g. sum of) costs
    # (todo)
    ...
    enumerator = LineSkeletonEnumerator( spec=spec, cands=skeleton_list, pools=priority_pools, iccs=iccs )
